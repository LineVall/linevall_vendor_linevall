function __print_linevall_functions_help() {
cat <<EOF
Additional LinneVallOS functions:
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the LineVall model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch linevall_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/linevall-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.linevall.device | grep -q "$LINEVALL_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $LINEVALL_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function linevallremote()
{
    ...
    git remote rm linevall 2> /dev/null
    ...
    then
        ...
        local PFX="LineVallOS/"
    else
        local PROJECT=$REMOTE
    fi

    local LINEVALL_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$LINEVALL_USER" ]
    then
        git remote add linevall ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add linevall ssh://$LINEVALL_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'linevall' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/LineVall/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.linevall.device | grep -q "$LINEVALL_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        linevallremote
        git push linevall HEAD:refs/heads/'$1'
    fi
    '
}

function linevallgerrit() {
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "linevallgerrit" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.linevall.device | grep -q "$LINEVALL_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/linevall/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $LINEVALL_FIXUP_COMMON_OUT ]; then
