...
func init() {
	android.RegisterModuleType("linevall_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = linevallExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = linevallExpandVariables(ctx, dep_file)
	...
	cmd := linevallExpandVariables(ctx, String(g.properties.Cmd))
